...
@testset "Verifier LTI feasible" begin
    obj_max, x = CLC.verify_candidate_lyapunov_function(method_s, A_list,
                                                        c_list, solver)
    @test abs(obj_max - 1.0) < 1e-9
    @test norm(x - [1.0, 1.0]) < 1e-9
    obj_max, x = CLC.verify_candidate_lyapunov_function(method_m, A_list,
                                                        c_list, solver)
    @test abs(obj_max - 1.0) < 1e-9
    @test norm(x - [1.0, 1.0]) < 1e-9
end
...
@testset "Verifier LTI infeasible" begin
    obj_max, x = CLC.verify_candidate_lyapunov_function(method_s, A_list,
                                                        c_list, solver)
    @test abs(obj_max) < 1e-9
    @test norm(x) < 1e-9 # Gurobi sol, validated with plot
    obj_max, x = CLC.verify_candidate_lyapunov_function(method_m, A_list,
                                                        c_list, solver)
    @test abs(obj_max) < 1e-9
    @test norm(x) < 1e-9 # Gurobi sol, validated with plot
